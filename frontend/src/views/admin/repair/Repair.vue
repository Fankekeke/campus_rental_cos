...
import spaceAdd from './RepairAdd.vue'
import spaceEdit from './RepairEdit.vue'
...
export default {
    ...
    columns () {
      return [{
        ...
        customRender: (text, row, index) => {
          switch (text) {
            case '0':
              return <a-tag color="yellow">未完成</a-tag>
            case '1':
              return <a-tag color="blue">已完成</a-tag>
            default:
              return '- -'
          }
        }
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/repair-record-info/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/repair-record-info/page', {
        ...params
      }).then((r) => {
